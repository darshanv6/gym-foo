...
learning_rate(float)
behaviour_policy(str)
policy_parameters(dict)

(list)
"""

def __init__(self,agent_name=None,env=gym.make('CartPole-v1'),
            number_episodes = 500,discounting_factor = 0.9,
            learning_rate=0.001,behaviour_policy = "epsilon_decay",
    ...
    self.agent_name="ddqa_"+str(time.strftime("%Y%m%d-%H%M%S"))
        if agent_name is None else agent_name
...
def _build_sequential_dnn_model(self);
    ...
    model.add(Dense(hidden_layer[0],input_dim = self.n_states,activation = 'relu'))
...
def _update_online_model(self,experience_tuple):
    ...
    if done_flag:
        ...
        action_values_for_next_state[action] = self.target_model.predict(next_state)[0]
...
def _reshape_state_for_model(self,state):
    return np.reshape(stae,[1,self.n_states])
...
def train_agent(self):
    ...
    for episode in range(self.n_episodes):
        ...
        for n_step in count():
            ...
            if episdoe % 2 == 0: self.plot_training_statistics()
            if episode % 5 == 0: self.save_model_weights()
            return self.trainingStats_rewards_in_each_episode
        
def replay_experience_from_memory(self):
    if self.memory.get_memory_size()<self.experience_replay_batch_size:
        return False
    experience_mini_batch = self.memory.get_radom_batch_for_replay(batch_size=self.experience_replay_batch_size)
...
def plot_training_statistics(self,training_statistics = None):
    ...
    episodes = np.arrange(len(self.trainingStats_steps_in_each_episode))
